...
public abstract class PoolSizeCalculator {
    ...
    private void calculateOptimalCapacity(BigDecimal targetQueueSizeBytes) {
        ...
        System.out.println("Target queue memory usage (bytes)（线程池所需的最大工作队列大小（字节））: " + targetQueueSizeBytes);
        System.out.println("createTask() produced " + creatTask().getClass().getName() + " which took " + mem
                + " bytes in a queue");
        System.out.println("Formula: " + targetQueueSizeBytes + " / " + mem);
        System.out.println("* Recommended queue capacity (bytes)（建议队列容量）: " + queueCapacity);
    }
    ...
    private void calculateOptimalThreadCount(long cpu, long wait, BigDecimal targetUtilization) {
        ...
        System.out.println("Formula: " + numberOfCPU + " * " + targetUtilization + " * (1 + " + waitTime + " / "
                + computeTime + ")");
        System.out.println("* Optimal thread count（最佳线程数）: " + optimalthreadcount);
    }
